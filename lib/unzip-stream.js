...
UnzipStream.prototype._prepareOutStream = function (vars, entry) {
  ...
  if (!fileSizeKnown) {
    ...
    var matcherStream = new MatcherStream(searchPattern, function (
      matchedChunk,
      sizeSoFar
    ) {
  ...
  if (isEncrypted || !isVersionSupported) {
    var message = isEncrypted
      ? "Encrypted files are not supported!"
      : "Zip version " +
        Math.floor(vars.versionsNeededToExtract / 10) +
        "." +
...
UnzipStream.prototype._readExtraFields = function (data) {
  ...
  while (index < data.length) {
    ...
    switch (vars.extraId) {
      ...
      case 0x5855:
        fieldType =
          vars.extraId === 0x000d ? "PKWARE Unix" : "Info-ZIP UNIX (type 1)";
        var offset = 0;
        if (vars.extraSize >= 8) {
